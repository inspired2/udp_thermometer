use std::{net::{UdpSocket, ToSocketAddrs}, sync::{Arc, Mutex}};
...
impl Thermometer {
    pub fn new(name: &str, addr: impl ToSocketAddrs) -> Result<Self, String> {
        ...
        let addr = addr.to_socket_addrs().map_err(|_| format!("unable to parse provided socket addr"))?.next().ok_or("ToSocketAddrs iterator is empty".to_owned())?;
        let socket = UdpSocket::bind(addr).map_err(|_| format!("unable to connect to socket addr: {}", addr))?;
        ...
        std::thread::spawn(move || {
            ...
            while let Ok(_) = socket.recv(&mut buf) {
                ...
                let mut temp = cloned_state.lock().expect("unable to acquire mutex lock on Thermometer.state");
                *temp = new_temp;
            }
        });
        Ok(Self {name: name.to_owned(),state})

    }
    pub fn get_temperature(&self) -> Result<Temperature, String> {
        let temperature_ref = self.state.lock().map_err(|_| format!("unable to acquire mutex lock on Thermometer.state"))?;
        Ok(temperature_ref.clone())
    }
...
impl From<[u8;5]> for Temperature {
    fn from(mut arr: [u8;5]) -> Self {
        ...
        match temp_kind {
            ...
            _ => panic!("Invalid temperature format (0 - Celsius, 1 - Fahrenheit). Received - {}", temp_kind)
        }
    }
}
impl Temperature {
